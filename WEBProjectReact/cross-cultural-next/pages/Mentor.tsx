...
const MentorComponentCore = React.memo(() => {
      ...
      if ("permissions" in navigator) {
        try {
          ...
          if (cameraPermission.state === "denied" || micPermission.state === "denied") {
            throw new Error(
              "Camera or microphone access was previously denied. Please reset permissions in browser settings.",
            )
          }
  ...
  const requestMediaPermissions = async (): Promise<boolean> => {
    try {
      ...
      if (mediaError.name === "NotAllowedError") {
        alert(
          'Camera/microphone access is required for video calls. Please click "Allow" when prompted and refresh the page.',
        )
      }
  ...
  // ✅ FIXED Enhanced endVideoCall with initialization flag reset and proper session cleanup
  ...
  // ✅ FIXED Enhanced end session with better state management and proper Firebase cleanup
    ...
    try {
      ...
      if (currentSessionPath && firebaseDb) {
        ...
        try {
          ...
          console.log("✅ Session status updated to ended in Firebase")
        } catch (firebaseError) {
          console.error("❌ Error updating session status in Firebase:", firebaseError)
        }
      }

      // ✅ CRITICAL FIX: Clear all session-related state immediately
      setActiveFirebaseSessionPath(null)
      activeSessionRef.current = null
      setChatMessages([])
      setIncomingVideoCall(null)
      ...
      console.log("✅ Session ended and all state cleared")
    } catch (err) {
      console.error("❌ Session cleanup error:", err)
    }
  }, [firebaseDb, myUserId, isVideoCallActive, endVideoCall])

  const setupSession = useCallback(
    async (path: string, sessionType: string) => {
        ...
        const sessionUnsubscribe = onValue(
          ...
          (snapshot) => {
            try {
              ...
              // ✅ FIXED: Better session end detection
              if (
                sessionStatus &&
                ...
                endSession()
              } else if (sessionStatus === "ended" && activeFirebaseSessionPath === path && !isVideoCallActive) {
                console.log("Session ended by remote party for our active session (simple status)")
                endSession()
              }
            } catch (err) {
  ...
  useEffect(() => {
    ...
    const setupListeners = async () => {
      ...
      try {
        ...
        const responsesUnsubscribe = onChildAdded(
          responsesRef,
          (snapshot) => {
            if (!isActive) return
            ...
            try {
              ...
              if (responseTime > 0 && responseTime < sessionStartThreshold) {
                console.log(
                  "Ignoring pre-session response:",
                  responseId,
                  "from",
                  new Date(responseTime).toLocaleString(),
                )
                return
              }
              ...
              console.log(
                ...
                response.type,
                responseId,
                responseTime ? `from ${new Date(responseTime).toLocaleString()}` : "(no timestamp)",
              )
  ...
  const sendRequest = useCallback(
    async (targetUserId: string, requestType: "chat" | "video" = "chat") => {
  ...
  const acceptRequest = useCallback(
    async (fromMentorId: string, sessionType: "chat" | "video", requestId: string) => {
  ...
  return (
    <div>
      ...
      >
        ...
        >
          <h3 className="text-secondary" style={{ margin: "0 0 15px 0" }}>
            Your Profile
          </h3>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "15px" }}>
            <div>
              <strong>Name:</strong> {myUserDetails.displayName}
            </div>
            <div>
              <strong>Role:</strong>{" "}
              <span
        ...
        {/* ✅ FIXED Incoming Video Call Notification with username display */}
          ...
          >
            <div>
              ...
              <p style={{ fontSize: "12px", color: "#6c757d" }}>
                Call ID: {incomingVideoCall.callId} | Your Role: {myRole}
              </p>
            </div>
          ...
          >
            ...
            >
              ...
              <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                {/* ✅ FIXED: Only show Start Video Call button for mentors */}
            ...
            >
              <strong>Debug:</strong> Session: {activeFirebaseSessionPath ? "✅" : "❌"} | Video Active:{" "}
              {isVideoCallActive ? "✅" : "❌"} | Local Stream: {localStream ? "✅" : "❌"} | Remote Stream:{" "}
              {remoteStream ? "✅" : "❌"} | Call Status: {callStatus || "None"}
            </div>
            ...
            {isVideoCallActive && (
              <div style={{ marginBottom: "25px" }}>
                ...
                >
                  ...
                  <div style={{ fontSize: "14px", color: "#6c757d" }}>Status: {callStatus || "Connected"}</div>
                </div>
